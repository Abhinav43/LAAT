...
def transformation(tensor_a, tensor_b, pad = False):
    
    """ Transform the input from two outputs into one"""
    
    if pad:
        tensor_b   = tensor_b.expand(tensor_a.size()[0],  
                                     tensor_b.size()[0], 
                                     tensor_b.size()[1])
    
    sum_tensor     = tensor_a + tensor_b
    concat_dim     = tensor_a.dim() - 1
    concat_tensor  = torch.cat((tensor_a, tensor_b), concat_dim)
