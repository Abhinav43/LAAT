...
class AttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.third_linears = nn.ModuleList([nn.Linear(self.size//2 +
    ...
    def forward(self, x, previous_level_projection=None, label_level=0):
        ...
        if self.attention_mode == "caml":
            ...
            weights            = layer_ops(x, self.gcn_1_layer, 
                                           self.gcn_1_data, 
                                           label_level, self.first_linears, con_dim = 0)
                        
            if self.args.concat_1_att:
                weights            = self.concat_1_att[label_level](weights)
            
            
            print("yeda",x.shape, weights.shape)
            weights           = x @ weights
        ...
        att_weights    = layer_ops(weights, self.gcn_2_layer, 
                                   self.gcn_2_data, 
                                   label_level, self.second_linears, con_dim = 1)
        ...
        gcn_weighted_output    = layer_ops(weighted_output, self.gcn_3_layer, 
                                   self.gcn_3_data, 
                                   label_level, self.third_linears, con_dim = 1, trans = False)
