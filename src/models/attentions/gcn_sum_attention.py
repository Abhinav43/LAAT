...
class AttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.concat_1_att    = nn.ModuleList([att_layer((self.d_a if self.d_a > n_labels[label_lvl] else n_labels[label_lvl]), 
                                                        self.size) for label_lvl in range(len(n_labels))])
        
        self.concat_2_att    = nn.ModuleList([att_layer(n_labels[label_lvl],
                                                        (self.d_a if self.d_a > n_labels[label_lvl] else n_labels[label_lvl]))
    ...
    def forward(self, x, previous_level_projection=None, label_level=0):
        ...
        if self.attention_mode == "caml":
            ...
            weights             = layer_ops_sum(x, self.gcn_1_layer, 
                                           self.gcn_1_data, 
                                           label_level, self.first_linears, device, con_dim = 0).transpose(1,2)
        ...
        att_weights    = layer_ops_sum(weights, self.gcn_2_layer, 
                                   self.gcn_2_data, 
                                   label_level, self.second_linears, device, con_dim = 1, trans = False).t()
        ...
        gcn_weighted_output    = layer_ops_sum(weighted_output, self.gcn_3_layer, 
                                   self.gcn_3_data, 
                                   label_level, self.third_linears, device, con_dim = 1, trans = False)
