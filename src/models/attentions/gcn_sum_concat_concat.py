...
adj_type   = 'occ_tboth'
emd_type   = 'dim_300_10_half_raw_w2v_avg_w2v_no_tfidf'
...
class AttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.gcn_3_layer = gcn_l(n_labels,  self.gcn_1_data[0][0].shape[-1],
                                 [self.size, self.size + level_projection_size], 
                                 inner_dims = 1024, 
                                 drop = self.args.gcn_drop, 
                                 att  = self.args.gcn_att, device = device)
        ...
        self.first_linears = nn.ModuleList([nn.Linear(self.size, 
                                                      self.n_labels[label_lvl], bias=False) 
                                            for label_lvl in range(self.n_level)])
        
        self.second_linears = nn.ModuleList([nn.Linear(self.n_labels[label_lvl], 
                                                       self.n_labels[label_lvl], bias=False) 
                                             for label_lvl in range(self.n_level)])
        ...
        self.first_linears1 = nn.ModuleList([nn.Linear(self.n_labels[label_lvl] * 2 + 
                                                       self.n_labels[label_lvl], 
                                                       self.n_labels[label_lvl], bias=False) 
                                             for label_lvl in range(self.n_level)])
        
        
        self.second_linears1 = nn.ModuleList([nn.Linear(self.n_labels[label_lvl] * 2 + 
                                                       self.n_labels[label_lvl], 
                                                       self.n_labels[label_lvl], bias=False) 
                                             for label_lvl in range(self.n_level)])
        ...
        self.third_linears1 = nn.ModuleList([nn.Linear(self.n_labels[label_lvl] * 2 + 
                                                       self.n_labels[label_lvl], 
                                                       self.n_labels[label_lvl], bias=False) 
                                             for label_lvl in range(self.n_level)])
    ...
    def _init_weights(self, mean=0.0, std=0.03) -> None:
        ...
        for first_linear1 in self.first_linears1:
            torch.nn.init.normal(first_linear1.weight, mean, std)
            if first_linear1.bias is not None:
                first_linear1.bias.data.fill_(0)
        ...
        for linear2 in self.second_linears1:
            torch.nn.init.normal(linear2.weight, mean, std)
            if linear2.bias is not None:
                linear2.bias.data.fill_(0)
        ...
        if self.attention_mode == "label" or self.attention_mode == "caml":
            for linear3 in self.third_linears1:
                torch.nn.init.normal(linear3.weight, mean, std)
    ...
    def forward(self, x, previous_level_projection=None, label_level=0):
        ...
        if self.attention_mode == "caml":
            ...
            weights            = transformation(self.first_linears[label_level].weight.t(), 
                                                self.gcn_1_layer[label_level](self.gcn_1_data[label_level]).t())
                        
            weights            = weights @ self.first_linears1[label_level].weight.t()
        ...
        att_weights    = transformation(self.second_linears[label_level].weight.t(), 
                                                self.gcn_2_layer[label_level](self.gcn_2_data[label_level]).t())
                
        att_weights    = att_weights @ self.second_linears1[label_level].weight.t()
        ...
        gcn_weighted_output    = transformation(self.third_linears[label_level].weight.t(), 
                                                self.gcn_3_layer[label_level](self.gcn_3_data[label_level]).t())
        
        gcn_weighted_output    = gcn_weighted_output @ self.third_linears1[label_level].weight.t()

        gcn_weighted_output    = gcn_weighted_output.t()
