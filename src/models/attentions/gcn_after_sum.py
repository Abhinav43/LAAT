...
class AttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.args.after_sum == 'cat':
            self.third_linearslast = nn.ModuleList([nn.Linear(self.n_labels[label_lvl]*2,
                                               self.n_labels[label_lvl], bias=True) for label_lvl in range(self.n_level)])
        elif self.args.after_sum == 'catsum':
            self.third_linearslast = nn.ModuleList([nn.Linear(self.n_labels[label_lvl]*2 + 
                                                          self.n_labels[label_lvl], 
                                                          self.n_labels[label_lvl],
                                                              bias=True) for label_lvl in range(self.n_level)])
        else:
            self.third_linearslast = None
    ...
    def forward(self, x, previous_level_projection=None, label_level=0):
        ...
        if self.attention_mode == "label" or self.attention_mode == "caml":
            ...
            weighted_output = after_sum_bottom(weighted_output, 
                                    ...
                                    self.gcn_1_data, 
                                    label_level, 
                                    self.args.after_sum, self.third_linearslast)
